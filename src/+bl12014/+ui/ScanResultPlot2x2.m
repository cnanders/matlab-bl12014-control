classdef ScanResultPlot2x2 < mic.Base
    ...
    methods
        ...
        function [ceData, ceFields] = getValuesStructFromCsvFile(this, cPath)
            ...
            try 
                [ceData, ceFields] = this.getValuesStructFromCsvFileV11(cPath); % 87 fields 2024.06.14 saveCSV timing
                return;
            catch mE
                
             end
            try 
                [ceData, ceFields] = this.getValuesStructFromCsvFileV10(cPath); % 86 fields 2024.06.11 saveCSV timing
                return;
            catch mE
            ...
            try 
                [ceData, ceFields] = this.getValuesStructFromCsvFileV8(cPath); % 81 fields 2023.03.28 saveCSV timing
                return;
            catch mE
            ...
            try 
                [ceData, ceFields] = this.getValuesStructFromCsvFileV7(cPath); % 80 fields 2022.02.xx smsSlowShutter timing
                return;
            catch mE
            ...
            try 
                [ceData, ceFields] = this.getValuesStructFromCsvFileV6(cPath); % 79 fields 2021.10.18
                return;
            catch mE
        ...
        % Before adding the timing data in 2020.09
        function [ceData, ceFields] = getValuesStructFromCsvFileV1(this, cPath)
        ...
        % After adding timing data in 2020.09
        function [ceData, ceFields] = getValuesStructFromCsvFileV2(this, cPath)
            
            hFile = fopen(cPath);
            
            % Get header fields
            cFormat = repmat('%q', 1, 70);
            ceHeader = textscan(...
                hFile, cFormat, 1, ...
                'delimiter', ',' ...
                ...'whitespace', '' ...
            ...
            cFormat = [...
                ...
                '%f%f%f%f%f%f%f%f%f%f', ... 50
                '%f%f%f%f', ... 54
                '%f%f%f%f%f%f%f%f%f%f%f%f', ... 66 new timing info
                '%{yyyy-MM-dd HH:mm:ss}D', ... !!! CAREFUL !!! here need to use DateTime format, not datestr format !! search datetime properties, the characters for month and year and maybe others are different. SO DUMB
                '%f%f%f' ... 
            ];
        ...
        function [ceData, ceFields] = getValuesStructFromCsvFileV4(this, cPath)
            
            hFile = fopen(cPath);
            
            % Get header fields
            cFormat = repmat('%q', 1, 72);
            ceHeader = textscan(...
                hFile, cFormat, 1, ...
                'delimiter', ',' ...
                ...'whitespace', '' ...
        ...
        function [ceData, ceFields] = getValuesStructFromCsvFileV6(this, cPath)
            
            hFile = fopen(cPath);
            
            % Get header fields
            cFormat = repmat('%q', 1, 79);
            ceHeader = textscan(...
                hFile, cFormat, 1, ...
                'delimiter', ',' ...
                ...'whitespace', '' ...
            ...
            cFormat = [...
                ...
                '%f%f%f%f%f%f%f%f%f%f', ... 50
                '%f%f%f%f', ... 54
                '%f%f%f%f%f%f%f%f%f%f%f%f%f%f', ... 68 (14 scan timing  info)
                '%{yyyy-MM-dd HH:mm:ss}D', ... !!! CAREFUL !!! here need to use DateTime format, not datestr format !! search datetime properties, the characters for month and year and maybe others are different. SO DUMB
                '%f', ... counts_dose_monitor
                '%f%f%f%f', ... dmi vib_x, vib_y, drift_x, drift_y (2021.05.26)
                '%f', ... dose_mj_cm_2
        ...
        function [ceData, ceFields] = getValuesStructFromCsvFileV7(this, cPath)
            
            hFile = fopen(cPath);
            
            % Get header fields
            cFormat = repmat('%q', 1, 80);
            ceHeader = textscan(...
                hFile, cFormat, 1, ...
                'delimiter', ',' ...
                ...'whitespace', '' ...
            ...
            cFormat = [...
                ...
                '%f%f%f%f%f%f%f%f%f%f', ... 50
                '%f%f%f%f', ... 54
                '%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f', ... 69 (15 scan timing  info after adding smsSlowShutter)
                '%{yyyy-MM-dd HH:mm:ss}D', ... !!! CAREFUL !!! here need to use DateTime format, not datestr format !! search datetime properties, the characters for month and year and maybe others are different. SO DUMB
                '%f', ... counts_dose_monitor
                '%f%f%f%f', ... dmi vib_x, vib_y, drift_x, drift_y (2021.05.26)
                '%f', ... dose_mj_cm_2
         ...
         function [ceData, ceFields] = getValuesStructFromCsvFileV8(this, cPath)
            
            hFile = fopen(cPath);
            
            % Get header fields
            cFormat = repmat('%q', 1, 81);
            ceHeader = textscan(...
                hFile, cFormat, 1, ...
                'delimiter', ',' ...
                ...'whitespace', '' ...
          ...
          function [ceData, ceFields] = getValuesStructFromCsvFileV9(this, cPath)
        ...
        function [ceData, ceFields] = getValuesStructFromCsvFileV11(this, cPath)
            
            hFile = fopen(cPath);
            
            % Get header fields
            cFormat = repmat('%q', 1, 87);
            ceHeader = textscan(...
                hFile, cFormat, 1, ...
                'delimiter', ',' ...
                ...'whitespace', '' ...
            ...
            cFormat = [...
                ...
                '%{yyyy-MM-dd HH:mm:ss}D', ... !!! CAREFUL !!! here need to use DateTime format, not datestr format !! search datetime properties, the characters for month and year and maybe others are different. SO DUMB
                '%f', ... counts_dose_monitor
                '%f%f%f%f', ... wafer_stalls, vibration_timeouts, focus anomalies, charge/ms 
                '%f%f%f%f', ... dmi vib_x, vib_y, drift_x, drift_y (2021.05.26)
                '%f', ... dose_mj_cm_2
                '%f%f', ... dz_height sensor, dz_wafer_fine
                '%f%f', ... z_height_sensor_target_nm, z_height_sensor_error_nm
            ...
            if contains(cPath, '.csv')
                ...
                try
                    ...
                    for n = 1 : length(ceData)
                        ...
                        % 2020.09 if field name is empty, we are loading
                        % an older log file that doesn't have this field
                        % populated.  This makes us robust to adding new
                        % fields 
                        if isempty(ceFields{n}) 
                            continue
                        end
